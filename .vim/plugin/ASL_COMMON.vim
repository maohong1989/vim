...
function! ASL_DetectCommentStype()
	...
	if g:GlobalFileType.flag == 1
		...
		let a:comment_name = expand("%")
		let a:suffix       = substitute(a:comment_name, '^.*\.', '', 'g')
		let b:FileType     = get(g:NoteType, a:suffix, '#')
	endif

	let ASSING = '^\(.\{-}\)\s*\(@\)\s*\(.*\)'
	let a:list = matchlist(b:FileType, ASSING)
	if empty(a:list)
		let b:TypeIndex      = 1
		...
		let b:TypeIndex      = 2
		let b:TypeLeftParam  = copy(a:list[1])
		let b:TypeRightParam = copy(a:list[3])
	endif
 	return {'TypeIndex':b:TypeIndex, 'TypeLeft':b:TypeLeftParam,
 				\ 'TypeRight':b:TypeRightParam}
endfunction
